...
import pyowm
owm = pyowm.OWM('a41a058c14f2b85d14f94ca183e15540')
...
def plot_line(days, min_t, max_t):
    ...
    plt.plot(days, max_t, color='black', linestyle='solid', linewidth=1, marker='*', markerfacecolor='red',
             markersize=15)
    plt.plot(days, min_t, color='black', linestyle='solid', linewidth=1, marker='*', markerfacecolor='green',
             markersize=15)
    ...
    plt.legend(["Maximum Temperature", "Minimum Temperature"], loc=4)
...
def plot_bars(days, min_t, max_t):
    ...
    days = dates.date2num(days)
    rcParams['figure.figsize'] = 6, 4
    min_temp_bar = plt.bar(days - 0.2, min_t, width=0.4, color='pink')
    max_temp_bar = plt.bar(days + 0.2, max_t, width=0.4, color='purple')
    plt.xticks(days)
    x_y_axis = plt.gca()
    xaxis_format = dates.DateFormatter('%d/%m')

    x_y_axis.xaxis.set_major_formatter(xaxis_format)
    plt.legend(["Minimum Temperature", "Maximum Temperature"], loc=4)
    plt.xlabel('Dates(dd/mm)')
    plt.ylabel('Temperature')
    plt.title('5-Day Weather Forecast')
